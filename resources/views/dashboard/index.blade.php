                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-white/90 font-garet mb-3 sm:mb-4 leading-relaxed text-sm sm:text-base line-clamp-2 sm:line-clamp-none">
                                ...
                                @else
                                    {{ $competition->name }} - {{ $user->university }}
                                @endif
                            </p>
                        </div>
                ...
                <div class="space-y-1 sm:space-y-2">
                    @if($user->link_drive_payment)
                        @if($status === 'verified')
                            ...
                            <p class="text-xs sm:text-sm text-[#6B7280] font-garet">Payment confirmed</p>
                        @elseif($status === 'pending')
                    ...
                    @else
                        ...
                        <p class="text-xs sm:text-sm text-[#6B7280] font-garet">Upload payment proof</p>
                    @endif
                </div>
            </div>
                            ...
                            <div class="ml-4 sm:ml-6 flex-1 min-w-0">
                                <div class="bg-gradient-to-br from-[#F8FBFC] to-white rounded-xl p-4 sm:p-5 border-2 border-[#D3EB9F]/30 shadow-md">
                                    ...
                                    <div class="flex flex-wrap items-center text-[#6B7280] font-garet text-xs sm:text-sm mb-2 sm:mb-3 gap-1">
                                        ...
                                        <span class="font-bold text-[#0F4C82]">{{ \Carbon\Carbon::parse($schedule['registration_start'])->format('d M Y') }}</span>
                                        <span class="mx-1">-</span>
                                        <span class="font-bold text-[#0F4C82]">{{ \Carbon\Carbon::parse($schedule['registration_end'])->format('d M Y') }}</span>
                                    </div>
                                    <p class="text-xs sm:text-sm text-[#6B7280] font-garet mb-2 sm:mb-3">
                                        Registration and document upload deadline
                                    </p>
                                    @if(now()->between(\Carbon\Carbon::parse($schedule['registration_start']), \Carbon\Carbon::parse($schedule['registration_end'])))
                                    ...
                                    @elseif(now()->lt(\Carbon\Carbon::parse($schedule['registration_start'])))
                            ...
                            <div class="ml-4 sm:ml-6 flex-1 min-w-0">
                                <div class="bg-gradient-to-br from-[#F8FBFC] to-white rounded-xl p-4 sm:p-5 border-2 border-[#D3EB9F]/30 shadow-md">
                                    ...
                                    <div class="flex flex-wrap items-center text-[#6B7280] font-garet text-xs sm:text-sm mb-2 sm:mb-3 gap-1">
                                        ...
                                        <span class="font-bold text-[#0F4C82]">{{ \Carbon\Carbon::parse($schedule['competition_start'])->format('d M Y') }}</span>
                                        <span class="mx-1">-</span>
                                        <span class="font-bold text-[#0F4C82]">{{ \Carbon\Carbon::parse($schedule['competition_end'])->format('d M Y') }}</span>
                                    </div>
                                    <p class="text-xs sm:text-sm text-[#6B7280] font-garet mb-2 sm:mb-3">
                                        Competition execution and judging period
                                    </p>
                                    @if(now()->between(\Carbon\Carbon::parse($schedule['competition_start']), \Carbon\Carbon::parse($schedule['competition_end'])))
                                    <div class="inline-flex items-center px-2 sm:px-3 py-1 bg-green-100 text-green-700 text-xs font-helvetica font-black rounded-full">
                                        <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-500 rounded-full mr-1.5 sm:mr-2 animate-pulse"></span>
                                        Ongoing
                                    </div>
                                    @elseif(now()->lt(\Carbon\Carbon::parse($schedule['competition_start'])))
                                    <div class="inline-flex items-center px-2 sm:px-3 py-1 bg-blue-100 text-blue-700 text-xs font-helvetica font-black rounded-full">
                                        <svg class="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
